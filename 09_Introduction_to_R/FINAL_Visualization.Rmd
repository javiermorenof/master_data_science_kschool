...
```
Además, el parámetro `lty` controla el tipo de línea (sólida, discontinua, etc) 

Con el ejemplo que se acaba de ver de `abline`, se puede ver que se ha dibujado una relación 1:1. Es decir, que cuando x=1 y=1, pero parece ser más una relación 1:3 o 1:4. Podríamos averiguar su relación lineal realizando una regresión lineal (lm), donde:
...
Teniendo estos resultados, ya podríamos adecuar la recta `abline` a la realidad:
...
Una de las maneras de ver qué Estados se encuentran dentro de un rango es con `which`, que devuelve un índice posicional.

Así, el siguiente código devuelve que el Estado con índice 5 en la lista es el que tiene un total de asesinatos > 1200.
```{r}
...
Sin el `which` hubiera funcionado también:
```{r}
...
```
Con este tipo de plot obtenemos una idea de cómo es la distribución de los datos: no simétrica, con el valor mas probable entre 0 y 200 y con algunos valores muy distintos del resto. Estas características serán las que miraremos más adelante cuando queramos caracterizar distribuciones de datos.
...
```
Como se verá a continuación, los boxplot forman parte de la estadística no paramétrica.

La estadística no paramétrica es más robusta que la paramétrica. El equivalente en estadística paramétrica a la mediana (estadística no paramétrica), es la media; y el equivalente a la desviación absoluta de la mediana (mad) en estadística paramétrica es la desviación estándar.
...
El problema estadístico de los test de normalidad es que utilizan la media como referencia y como se ha visto en el ejemplo de murders, muchas veces no es significativa. Las variables discretas generalmente no siguen una distribución normal, mientras que la normalidad es más común en variables continuas.
...
Podemos ver qué regiones son outliers:
```{r}
...
Cuando tenemos muchos datos lo primero que solemos querer hacer es intentar resumir la información en un sólo número. En principio esto debería de darnos una idea acerca de algunas de las características importantes de nuestros datos. Qué medidas de sumarización se nos ocurren? La media, la mediana, la desviación estandar...ahora vamos a ver cuándo usar cada una de ellas, cuándo son y cuándo no son informativas.
...
sd(babies$bwt)
```
**IMPORTANTE**: Recordando cómo era su distribución (simétrica, un sólo valor más probable...) parece que estos valores son bastantes informativos acerca de estos datos
...
```
Un tipo de plot que no habíamos visto y que también es muy informativo es el que compara los quantiles de nuestros datos con los de una distribucion normal. Este es el **qqplot**. Si los datos se describen en la línea, la distribución es normal. Como se puede ver este no es el caso:
```{r}
...
p <-  murders %>% 
  #Dentro de aes en ggplot pongo cuál es el eje x y cuál es el eje y.
  #Con ggplot solo no es suficiente para que nos muestre el gráfico, ya que le falta añadirle la geometría indicando qué tipo queremos.
  #Con lablel=abb le pido que me ponga etiquetas
  #Con el '+' indico que voy a añadir má cosas.
  ggplot(aes(population/10^6, total, label = abb))+
  #Con geom_point indico que quiero que sea un scatterplot y le digo que me los coloree por region y el tamaño de los puntos
  #Técnciamente podría poner aquí debajo lo que puse en el aes de arriba de ggplot, pero es desaconsejable, ya que ponerlo en ggplot es como poner los mimbres y pemite reutilizarlo
  geom_point(aes(col=region), size = 3) +  
  #Con geom_text le digo que me escriba las labels que puse en ggplot
  geom_text(nudge_x = 0.05) +
  #Aquí pido que me escale a base 10 el eje X y el eje Y
  scale_x_log10() +
  scale_y_log10() +
...
Puedo pedir que me ponga los tamaños escalados al total:
```{r,warning=FALSE}
data(murders)
...
p <-  murders %>% 
  #Aquí me colorea también el texto y no solo los puntos, porque el 'col=region' está en ggplot y no en geom_points
