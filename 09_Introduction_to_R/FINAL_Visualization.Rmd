...
Sin embargo, como se ve parece que la relación no es tanto 1:1 sino 1:3
...
Podríamos verlo mejor con una regresión lineal:
...
Si estroy trabajando en la línea de comandos y quisiera añadir algo en el gráfico tendría que usar `par(new=T)`.
...
O en su lugar usar `points` en vez de plot, que añade elementos al gráfico
```{r}
points(150,0)
```
...
```
La estadística no paramétrica es más robusta que la paramétrica.
...
p <-  murders %>% 
  ...
  #Esto no es suficiente para que nos lo muestre, ya que le falta añadirle la geometría
  #Con lablel=abb le pido que me ponga etiquetas
  #Con el '+' indico que voy a añadir má cosas.
  ggplot(aes(population/10^6, total, label = abb))+
  #Aquí ya añadimos la geometría
  #Con geom_point indico que quiero que sea un scatterplot y le digo que me los coloree por region y que el tamaño de los puntos
...
ara el siguiente plot, describe los datos, la geometria y el aesthetic mapping.
```{r,warning=FALSE}
data(murders)
