...
```{r}
example_2 <- gapminder %>% filter(year=="2015" & country %in% c("Poland","South Korea")) %>% select(c(country, infant_mortality))
example_2
```
...
Con el '-' que se ponde delante de year se pide que seleccione todo menos year
```{r}
tab.USA<-tab %>% 
  filter(year=="2015") %>% 
  select(-year) %>% 
...
¿Cómo puedo ver qué nombres hay en continent? DE dos maneras
```{r}
...
Mediante la columna mutate estoy añadiendo una nueva columna con el murder rate
```{r}
...
0. Calcula la media de presión sistólica y diastólica para hombres y mujeres en cada rango de edad:
```{r}
library(NHANES)
data(NHANES)
NHANES %>% group_by(Gender, AgeDecade, .drop=TRUE) %>% summarize(avgSys=mean(BPSysAve, na.rm=TRUE), avgDia=mean(BPDiaAve, na.rm=TRUE))
```
Veo que hay NA en AgeDecade, así que los voy a eliminar con `!is.na`, es decir, que es lo contrario a na.

```{r}
library(NHANES)
data(NHANES)
NHANES %>% filter(!is.na(AgeDecade)) %>% group_by(Gender, AgeDecade, .drop=TRUE) %>% summarize(avgSys=mean(BPSysAve, na.rm=TRUE), avgDia=mean(BPDiaAve, na.rm=TRUE))
```

O mejor, para los que son na les voy a pedir que me enseñe edad, por si pudiera meterlo en un rango.

```{r}
library(NHANES)
data(NHANES)
NHANES %>% filter(is.na(AgeDecade)) %>% select(Age)
```
Y veo que si tienen edades, así que podría incluirlos en +70, aunque es sospechoso de que para todos aquellos que no tengan resultados de esa variable les hayan metido 80 directamente.

Ojo que en el código veo que en el group_by no hay 'c(..)' porque no estoy concatenando vectores.
...
Country tiene el signo '-' (-country) poorque quiero que me pivote todo menos el country
...
billboard %>% 
  pivot_longer(
    cols = starts_with("wk"), 
    names_to = "week",
    #Con names_prefix indico que prefijo quiero quitar
    names_prefix = "wk",
...
En names pattern se pone mediante una expresión regular el patrón que va a usar para separar entre los grupos de 'names_to'.: Significa que:
...
- (.*). La siguiente aprte, como está entre paréntesis, coge tantos elementos como encuentre hasta el siguiente guión y los meterá en diagnosis, ya que es el primer grupo que coge.
- (.). Luego coge un punto
